...
public class Secp256k1Context : Context {
    ...
    public func sign(data: [UInt8], privateKey: PrivateKey) -> String {
        let ctx = secp256k1_context_create(UInt32(SECP256K1_CONTEXT_SIGN))
        var sig = secp256k1_ecdsa_signature()

        var msgDigest = hash(data: data)
        _ = msgDigest.withUnsafeMutableBytes { (msgDigestBytes) in
        ...
        secp256k1_ecdsa_signature_parse_compact(ctx!, &compactSig, input)
    ...
    public func getPublicKey(privateKey: PrivateKey) -> PublicKey {
        ...
        _ = secp256k1_ec_pubkey_create(ctx!, &pubKey, privateKey.getBytes())
